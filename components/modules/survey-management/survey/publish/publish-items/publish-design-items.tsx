...
import { ChevronsUpDown, ListTodo, VectorSquare } from "lucide-react"
...
export const PublishDragItems = () => {
  // * HOOKS
  const { isPending, data, error } = trpc.survey.getPublishComponents.useQuery()
  ...
  const CreateDragItem: React.FC<{
    ...
    return item?.map((comp) => {
      const accept = comp?.subComponentType
      const isCategory = comp?.subComponentType === "Category"
      return (
        <Draggable
          key={comp?.id}
          id={`${accept}-${comp?.id}`}
        ...
        >
          ...
          >
            ...
            <small className="truncate flex-1">{comp?.name}</small>
          </Button>
        </Draggable>
      )
    })
